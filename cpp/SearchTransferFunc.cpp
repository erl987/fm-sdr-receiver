...
void Core::Processing::Filter::Solver::GetFuncVal(const alglib::real_1d_array &currFilterOrder, alglib::real_1d_array &currTransWidthResidual, void *ptr)
{
	...
	for (int i=0; i < filterOrders.length(); i++) {
		transWidths(i) = CFIRfilter<double, double>::CalcTransitionWidth( static_cast<int>( filterOrders(i) ), normCutoffFreq, samplingFreq );
	}
	...
	currTransWidthResidual(0) = abs( currTransWidth * samplingFreq - goalTransWidth );
}
